...
class Noduledetection(DetectionAlgorithm):
    def __init__(self, input_dir, output_dir, train=False, retrain=False, retest=False):
        ...
        #------------------------------- LOAD the model here ---------------------------------
        #self.test_case = {"path": "test/test.mha"}
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.input_train_path, self.output_path = input_dir, output_dir
        ...
        if not (train or retest):
            ...
            torch.load(
                Path("/opt/algorithm/model.pth") if execute_in_docker else Path("./model/model_fsrcnn_1.pth"),
        ...
        if retest:
            ...
            self.model.load_state_dict(
            torch.load(
                Path('./model/model_retrained.pth'),
                map_location=self.device,
                )
            ) 
    ...
    def train(self, num_epochs = 1):
        ...
        self.model.train() 
        input_dir = self.input_train_path
        ...
        for epoch in range(num_epochs):
            ...
            torch.save(self.model.state_dict(), os.path.join(self.output_path, 'model_fsrcnn_' + str(num_epochs) + '.pth'))
    ...
    def format_to_GC(self, np_prediction, spacing) -> Dict:
        ...
        for i, bb in enumerate(np_prediction['boxes']):
            ...
            x_min, y_min, x_max, y_max  = round(x_min, 2), round(y_min, 2), round(x_max, 2), round(y_max, 2) #保留两位小数
    ...
    def predict(self, *, input_image: SimpleITK.Image) -> DataFrame:
        ...
        for j in range(len(image_data)):
            ...
            #print(prediction) #xmin,ymin,xmax,ymax
